...
class FrontController extends Controller
{
    ...
    public function homePage()
    {
        ...
        $data['about'] = About::query()->with([
            'image', 'galleries' => function ($q) {
                $q->select(['id', 'about_id', 'sort'])
                    ->with(['image'])
                    ->orderBy('sort', 'ASC')->limit(2);
            },
        ])->first();
        ...
        $data['serviceCategories'] = PostCategory::query()->with('image')->where('type', PostCategory::TYPE_SERVICE)->where('show_home_page', 1)->get();

        $data['services'] = Service::query()
            ->whereHas('category', function ($query) {
                $query->where('type', PostCategory::TYPE_SERVICE);
            })->with(['image'])
            ->where('show_home_page', 1)
        ...
        $data['posts'] = Post::query()->with('image')->where('status', 1)->latest()->limit(6)->get();
    ...
    public function getListRooms()
    {
        ...
        $data['banner'] = Banner::query()->with('image')->where('type', 1)->first();
    ...
    public function about_page(Request $request)
    {
        ...
        return view('site.about_page', compact('about', 'partners', 'services'));
    }
    ...
    public function getServiceDetail(Request $request, $slug)
    {
        ...
        $otherServices = Service::query()->where('cate_id', $service->cate_id)->whereNotIn('id', [$service->id])
            ->where('status', 1)
            ->latest()->get();
        $categoryServices = PostCategory::query()->with(['services' => function ($q) {
            $q->select(['id', 'cate_id', 'name', 'slug'])->where('status', 1)->orderBy('created_at', 'desc')->limit(6);
        }, 'image'])->where('id', $service->cate_id)->firstOrFail();
    ...
    public function blogDetail(Request $request, $slug)
    {
        ...
        $categories = PostCategory::query()->withCount('posts')->where('type', 1)->get();
        // $service_cates = PostCategory::query()->with('image')->where('type', PostCategory::TYPE_SERVICE)->get();
        $services = Service::query()
            ->whereHas('category', function ($query) {
                $query->where('type', PostCategory::TYPE_SERVICE);
            })->with(['image'])
        ...
        return view('site.blog_detail', compact('blog', 'otherBlogs', 'categoryBlog', 'categories', 'services'));
    }
    ...
    {
        $banner = new Banner();
        $banner->title = 'Banner trang hạng phòng';
        $banner->created_by = 1;
        $banner->updated_by = 1;
        $banner->type = 1;
